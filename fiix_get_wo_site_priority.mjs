...
if (!base || !appKey || !accessKey || !secretKey || !code) {
  console.error("Usage: set FIIX_* env vars, then: node fiix_get_wo_site_priority.mjs <WorkOrderCode>");
  process.exit(1);
}
...
const body = {
  ...
  className: "WorkOrder",
  // Keep the fields list tight to avoid the “Custom Fields…” error you saw earlier
  fields: "id,strCode,intSiteID,intPriorityID,intWorkOrderStatusID,strDescription",
...
console.log("WorkOrder:", data.objects?.[0]);