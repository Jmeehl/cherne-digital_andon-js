...
const code = process.argv[2]; // e.g. 97448
...
if (!code) {
  console.error("Usage: node fiix_get_wo_status_id.mjs <WorkOrderCode>");
  process.exit(1);
}

// Build Fiix API URL (per no-SDK guide)
const ts = Date.now();
...
// Find the WO by strCode and return status ID
const body = {
  _maCn: "FindRequest",
  clientVersion: { major: 2, minor: 8, patch: 1 },
  className: "WorkOrder",
  fields: "id,strCode,intWorkOrderStatusID,strDescription",
...
// Fiix errors returned inside body
if (data?.error) {
...
if (!obj) {
  console.log("No WorkOrder found for strCode =", code);
  process.exit(0);
}

console.log("WorkOrder found:");
console.log("  id:", obj.id);
console.log("  strCode:", obj.strCode);
console.log("  intWorkOrderStatusID:", obj.intWorkOrderStatusID);
console.log("  strDescription:", obj.strDescription);